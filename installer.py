...
GITHUB = "Valen7440/Collector/contents/"
PACKAGE_PATH = "ballsdex/packages/collector"
PACKAGE_FILES = [
    "__init__.py", 
    "cog.py",
    "transformers.py"
]
TORTOISE_MODELS = ["collector_models.py"]
DJANGO_APP_FILES = {
    "collector_app": [
        "__init__.py", 
        "admin.py", 
...
os.makedirs("ballsdex/packages/collector", exist_ok=True)
os.makedirs("admin_panel/collector_app", exist_ok=True)
...
async def install_package_files():
    ...
    async with aiohttp.ClientSession() as session:
        for index, file in enumerate(files):
            data = await fetch_github_file(session, f"https://api.github.com/repos/{GITHUB}/collector/{file}")
...
await install_django_app_files("collector_app")
await install_package_files()
await add_package(PACKAGE_PATH.replace("/", "."))
await add_model("ballsdex.core.collector_models", ModelType.TORTOISE)
await add_model("collector_app", ModelType.DJANGO)
